...
import kotlinx.android.synthetic.main.activity_value_pairs.*
import kotlinx.android.synthetic.main.item_main_news.*

class NewsDetailPlainActivity : ToolbarActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_value_pairs)
        setTitle("资讯详情")
        setBack(true)
        val news = intent.getSerializableExtra(Extras.DATA) as NewsBean?
        news?.apply {
            item_sub.visibility = View.VISIBLE
            setSubview(this)
...
//            13 -> set13(map, data)
    ...
    companion object {
        fun start(ctx: Activity?, news: NewsBean) {
            val intent = Intent(ctx, NewsDetailPlainActivity::class.java)
